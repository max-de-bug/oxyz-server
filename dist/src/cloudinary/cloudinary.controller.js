"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CloudinaryController = void 0;
const common_1 = require("@nestjs/common");
const cloudinary_service_1 = require("./cloudinary.service");
const auth_guard_1 = require("../auth/guards/auth.guard");
let CloudinaryController = class CloudinaryController {
    cloudinaryService;
    constructor(cloudinaryService) {
        this.cloudinaryService = cloudinaryService;
    }
    async listFolders() {
        return this.cloudinaryService.listFolders();
    }
    async listSubFolders(folder) {
        return this.cloudinaryService.listSubFolders(folder);
    }
    async getResources(folder, maxResults, nextCursor) {
        const searchOptions = {
            expression: `folder:${folder}`,
            max_results: maxResults ? parseInt(maxResults) : 100,
            next_cursor: nextCursor,
        };
        return this.cloudinaryService.searchResources(`folder:${folder}`, searchOptions);
    }
    async searchResources(query, maxResults, nextCursor) {
        return this.cloudinaryService.searchResources(query, {
            max_results: maxResults ? parseInt(maxResults) : 100,
            next_cursor: nextCursor,
        });
    }
};
exports.CloudinaryController = CloudinaryController;
__decorate([
    (0, common_1.Get)('folders'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], CloudinaryController.prototype, "listFolders", null);
__decorate([
    (0, common_1.Get)('folders/:folder'),
    __param(0, (0, common_1.Param)('folder')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], CloudinaryController.prototype, "listSubFolders", null);
__decorate([
    (0, common_1.Get)('resources'),
    __param(0, (0, common_1.Query)('folder')),
    __param(1, (0, common_1.Query)('max_results')),
    __param(2, (0, common_1.Query)('next_cursor')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String]),
    __metadata("design:returntype", Promise)
], CloudinaryController.prototype, "getResources", null);
__decorate([
    (0, common_1.Get)('search'),
    __param(0, (0, common_1.Query)('query')),
    __param(1, (0, common_1.Query)('max_results')),
    __param(2, (0, common_1.Query)('next_cursor')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String]),
    __metadata("design:returntype", Promise)
], CloudinaryController.prototype, "searchResources", null);
exports.CloudinaryController = CloudinaryController = __decorate([
    (0, common_1.Controller)('cloudinary'),
    (0, common_1.UseGuards)(auth_guard_1.SupabaseAuthGuard),
    __metadata("design:paramtypes", [cloudinary_service_1.CloudinaryService])
], CloudinaryController);
//# sourceMappingURL=cloudinary.controller.js.map